...
class ReportTax(models.AbstractModel):
    ...
    def _sql_from_amls_one(self):
        sql = """SELECT "account_move_line".tax_line_id, amount_currency
                    FROM %s
                    WHERE %s GROUP BY "account_move_line".tax_line_id, amount_currency"""
    ...
    def _sql_from_amls_two(self):
        sql = """SELECT r.account_tax_id, amount_currency
                  FROM %s
                  INNER JOIN account_move_line_account_tax_rel r ON ("account_move_line".id = r.account_move_line_id)
                  INNER JOIN account_tax t ON (r.account_tax_id = t.id)
                  WHERE %s GROUP BY r.account_tax_id , amount_currency"""
